...
import { fetchNewestCars } from "@/features/cars/carsSlice";
...
export default function NewestCars() {
  const dispatch = useDispatch();
  const favorites = useSelector((state) => state.favorites.items);
  const { newestCars: newest, newestStatus, error } = useSelector((state) => state.cars);

  useEffect(() => {
    dispatch(fetchNewestCars());
  }, [dispatch]);
        ...
        <div className="relative carousel-container overflow-hidden">
          {newestStatus === "loading" ? (
            ...
            <div className="text-center py-8">
              <p className="text-red-500 mb-2">Failed to load newest cars: {error}</p>
              <button 
                onClick={() => dispatch(fetchNewestCars())}
            ...
            <Slider {...settings}>
              {newest.map((car) => (
                <Link key={car.id} href={`/product/${car.id}`} className="px-2 block">
